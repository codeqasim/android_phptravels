...
import com.ajubia.Adapters.ListingAdapters;
...
import com.ajubia.Network.Parser.MainMenuParse;
import com.ajubia.Network.Post.PostHotelRequest;
import com.ajubia.R;
        ...
        ota_id= otaModel.getOta_id();
                    ...
                    public void onResponse( Object response ) {
                        ...
                        try {
                            parentObject = new JSONObject(response.toString());
                                ...
                                if (offest == 1) {
                                    total_offest = parentObject.getInt("total_records");
                                }
                                ...
                                for (int i = 0; i < parentArray.length(); i++) {
                                    ...
                                    hm.setName(finalObject.getString("company_name"));
                                    hm.setPrice(finalObject.getString("price"));
                                    hm.setStarsCount(finalObject.getInt("rating"));
                                    hm.setThumbnail(finalObject.getString("image"));
                                    hm.setcurrCode(finalObject.getString("country_id"));
                                    hm.setLocation(finalObject.getString("address"));
                                    hm.setid(finalObject.getInt("id"));
                                    hm.setHotelSlug(finalObject.getString("hotel_slug"));
                                    list_models.add(hm);
                                }
                ...
                Response.ErrorListener errorListener = new Response.ErrorListener() {
                    ...
                    public void onErrorResponse( VolleyError error ) {
                        Log.d("HotelResponse", error.getMessage());
                    }
                };

                PostHotelRequest postHotelRequest = new PostHotelRequest(hotel_data.getLocation(), ota_id, hotel_data.getFrom(), hotel_data.getTo(), listener, errorListener);
                RequestQueue requestQueue1 = SingleTonRequest.getmInctance(this).getRequestQueue();
                requestQueue1.add(postHotelRequest);
